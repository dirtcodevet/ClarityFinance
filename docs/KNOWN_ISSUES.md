...
### Issue #9: Budget Edits Apply to Wrong Month (Carry-Forward Fork Missing)
**Resolution:**
- Budget edits now fork records into the active month when the source record originated in an earlier month.
- Future-month edits no longer overwrite past data, and updates render immediately after save.
