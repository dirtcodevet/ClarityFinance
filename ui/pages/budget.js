...
function toCreatePayload(record) {
...
async function saveAccount() {
  ...
  const changes = { bank_name: name, account_type: type, starting_balance: balance, monthString: state.currentMonth };
  const result = await budgetApi.updateAccount(id, changes);
  if (!result.ok) { showError(result.error.message); return; }
  if (result.meta?.forked) {
...
async function saveIncomeSource() {
  ...
  const changes = { source_name: name, income_type: type, amount, account_id: accountId, pay_dates: datesToJson(datesStr), monthString: state.currentMonth };
...
async function saveCategory() {
  ...
  const changes = { name, bucket_id: bucketId, monthString: state.currentMonth };
...
async function savePlannedExpense() {
  ...
  if (!expense) return;
  const data = { description, amount, bucket_id: expense.bucket_id, category_id: categoryId, account_id: accountId, due_dates: datesToJson(datesStr), is_recurring: isRecurring, monthString: state.currentMonth };
  if (endDate) data.recurrence_end_date = endDate;
  ...
  if (result.meta?.forked) {
    const payload = toCreatePayload(result.data);
    if (payload) {
      recordUndoAction({ type: 'create', entity: 'expense', id: result.data.id, payload });
    }
  } else {
    recordUndoAction({
      type: 'update',
...
async function saveGoal() {
  ...
  const changes = { name, target_amount: target, target_date: date, funded_amount: funded, monthString: state.currentMonth };
